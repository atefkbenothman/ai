...
export const codeSnippetSchema = z.object({
  snippets: z.array(
    z.object({
      language: z.string().describe("programming language the code was written in"),
...
export const ghPullRequestSchema = z.object({
  ...
  body: z.string().describe("detailed description of all modifications, including: what changes were made and why, which files were modified, any important implementation details, how to test the changes"),
  files: z.array(
    z.object({
      path: z.string().describe("full path of the file"),
      code: z.string().describe("complete file content with changes - NEVER truncate")
    })
  )
})
